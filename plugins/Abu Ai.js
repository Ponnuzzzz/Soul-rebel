...
Asena.addCommand({ pattern: 'abuser on ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.abuser on / off' }, (async (message, match) => {
    var abuser_status = `${conf.TALKING_ABU}`
    if (match[1] == 'on') {
        if (abuser_status == 'true') {
